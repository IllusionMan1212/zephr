...
init_freetype :: proc(font_path: cstring) -> i32 {
  ...
  gl.TexImage2D(gl.TEXTURE_2D, 0, gl.RED, cast(i32)tex_width,
      cast(i32)tex_height, 0, gl.RED, gl.UNSIGNED_BYTE,
      raw_data(pixels))
...
init_fonts :: proc(font_path: cstring) -> i32 {
  ...
  for i in 32..<128 {
    for j in 0..<4 {
      ...
      set_vec2f(font_shader, cstring(raw_data(text)), zephr_ctx.font.characters[i].tex_coords[j].x, zephr_ctx.font.characters[i].tex_coords[j].y)
    }
  }
...
draw_text :: proc(text: string, font_size: u32, constraints: UiConstraints, color: Color, alignment: Alignment) {
  ...
  gl.BindBuffer(gl.ARRAY_BUFFER, font_instance_vbo)
  gl.BufferData(gl.ARRAY_BUFFER, size_of(GlyphInstance) * len(glyph_instance_list), raw_data(glyph_instance_list), gl.DYNAMIC_DRAW)
...
add_text_instance :: proc(batch: ^GlyphInstanceList, text: string, font_size: u32, constraints: UiConstraints, color: Color, alignment: Alignment) {
  glyph_instance_list := get_glyph_instance_list_from_text(text, font_size, constraints, color, alignment)
