...
stack_set :: proc(stack: ^Stack($S), $T: typeid, new_value: T) where type_of(new_value) == T {
    node := stack.free
    if node == nil {
...
stack_push :: proc(stack: ^Stack($S), $T: typeid, new_value: T) where type_of(new_value) == T {
    node := stack.free
    if node == nil {
        node = new(S, build_allocator())
    } else {
