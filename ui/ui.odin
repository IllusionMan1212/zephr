...
px           :: proc(value, strictness: f32) -> Size {return {.Pixels, value, strictness}}
text_dim     :: proc(padding, strictness: f32) -> Size {return {.TextContent, padding, strictness}}
percent      :: proc(value, strictness: f32) -> Size {return {.PercentOfParent, value, strictness}}
children_sum :: proc(strictness: f32) -> Size {return {.ChildrenSum, 0, strictness}}

create_box_with_id :: proc(flags: Flags, id: Id, caller := #caller_location) -> ^Box {
...
box_equip_display_string :: proc(box: ^Box, str: string) {
    //ProfBeginFunction();
...
init :: proc() {
    ...
    ui_state.drawables = make([dynamic]DrawableInstance, 0, 128, context.temp_allocator)

    init_nil_stacks()
    init_drawing()
}

begin_build :: proc(window_size: m.vec2) {
    init_stacks()
    // TODO:
    //- rjf: build top-level root
    {
        set_next_fixed_width(window_size.x)
        set_next_fixed_height(window_size.y)
        set_next_layout_axis(.X)
        root := create_box_with_id({}, 55120055)
        push_parent(root)
        ui_state.root = root
    }
}
...
draw :: proc(projection: m.mat4) {
    ...
    iter_children :: proc(node: ^Box) {
        for child := node.first; child != nil; child = child.next {
            ...
            if .DrawBackground in child.flags {
                instance_rect(child.rect, child.background_color)
            }
            iter_children(child)
        }
    }
    ...
    //for box := root.first; box != nil; box = box.next {
    //    // Draw the background
    //    if .DrawBackground in box.flags {
    //        instance_rect(box.rect, box.background_color)
    //    }
    //    //if .DrawText in box.flags {
    //    //    //instance_text(box.rect)
    //    //}
    //}

    draw_instances(projection)
    clear(&ui_state.drawables)
}
...
init_nil_stacks :: proc() {
    ...
    ui_state.pref_width_nil_stack_top.v = px(250, 1)
    ui_state.pref_height_nil_stack_top.v = px(30, 1)
